...
def execute_command(cmd, vfs, is_script=False):
    ...
    try:
        ...
        # КОМАНДА TAC - выводит содержимое файла в обратном порядке (ЭТАП 4)
        elif command == "tac":
            if not args:
        ...
        # КОМАНДА DU - показывает размер файлов и директорий (ЭТАП 4)
        elif command == "du":
            if args:
                ...
                if dir_path in vfs.dirs:
                    total_size = vfs.calculate_directory_size(dir_path.rstrip('/') or '/')
...
def main():
    ...
    if args.script:
        ...
        print("Эмулятор командной оболочки (Этап 4). Введите 'exit' для выхода.")
